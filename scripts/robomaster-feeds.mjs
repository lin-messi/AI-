...
// —— 论文纳入过滤 ——
// arXiv 查询的分类（机器人 / 视觉 / 机器学习）。各分类「分开查询」再合并去重，提高召回（Q21）。
export const ARXIV_CATS = ["cs.RO", "cs.CV", "cs.LG"];

// 论文必须命中至少一个「主题关键词」（与 RoboMaster 算法强相关的视觉/机器人主题）。
// 已扩充视觉/检测/位姿相关词（CVPR 常见方向：6-DoF 位姿、单目、BEV、深度估计、开放词表、transformer 检测等）（Q8/Q64）。
export const PAPER_TOPIC_KW = [
  "object detection", "instance segmentation", "semantic segmentation",
  "keypoint detection", "pose estimation", "6-dof pose", "6dof pose", "6d pose",
  "monocular 3d", "monocular depth", "depth estimation", "open-vocabulary",
  "transformer detector", "real-time detection", "oriented object detection",
  "rotated object detection", "small object detection",
  "multi-object tracking", "re-identification", "bird's eye view", "bev perception",
...
// 「强主题词」：只要标题/摘要命中其一，即使缺机器人语境也直接纳入（Q19）。
// 仅收录区分度极高、与 RoboMaster 强绑定的词，避免误召回。
export const STRONG_TOPIC_KW = [
  "robomaster", "robo master", "armor plate", "armor detection", "armor recognition",
  "auto-aim", "auto aim", "autoaim", "power rune", "energy mechanism",
  "装甲板", "自瞄", "能量机关",
];

// —— 顶会识别（需求1）——
// 论文若被顶会接收，arXiv 的 comment 字段常含 "Accepted to CVPR 2025" 等；
// 也兜底扫描标题/摘要（Q9/Q62）。命中顶会的论文加权（Q6）并在卡片展示徽章（Q5）。
export const CONF_RECENT_YEARS = 3; // 仅识别近 N 年的顶会（Q10）
export const CONFERENCES = [
  // 视觉顶会（vision=true：可豁免机器人语境硬要求，Q12）
  { key: "CVPR", re: /\bCVPR\b/i, weight: 6, vision: true },
  { key: "ICCV", re: /\bICCV\b/i, weight: 6, vision: true },
  { key: "ECCV", re: /\bECCV\b/i, weight: 6, vision: true },
  { key: "WACV", re: /\bWACV\b/i, weight: 5, vision: true },
  // 机器人顶会
  { key: "ICRA", re: /\bICRA\b/i, weight: 5, vision: false },
  { key: "IROS", re: /\bIROS\b/i, weight: 5, vision: false },
  { key: "RSS", re: /Robotics:?\s*Science\s+and\s+Systems/i, weight: 5, vision: false },
  // 机器学习顶会（权重较低，Q4）
  { key: "NeurIPS", re: /\bNeurIPS\b|\bNIPS\b/i, weight: 3, vision: false },
  { key: "ICML", re: /\bICML\b/i, weight: 3, vision: false },
  { key: "ICLR", re: /\bICLR\b/i, weight: 3, vision: false },
  { key: "AAAI", re: /\bAAAI\b/i, weight: 3, vision: false },
];

// —— 相关度评分（需求3）——
// 离线在抓取时算好，写入条目的 relevance 字段，前端直接按它排序（Q34）。
// 关键词分三档权重（Q35）：high=RM 强绑定、mid=核心算法主题、low=通用机器人/视觉语境。
export const RELEVANCE_WEIGHTS = {
  high: [
    "robomaster", "robo master", "armor plate", "armor detection", "armor recognition",
    "auto-aim", "auto aim", "autoaim", "power rune", "energy mechanism", "light bar",
    "anti-spin", "anti-top", "gimbal", "ballistic",
    "装甲板", "自瞄", "能量机关", "灯条", "符叶", "步兵视觉", "上位机", "反陀螺", "弹道", "云台",
  ],
  mid: [
    "object detection", "instance segmentation", "keypoint", "pose estimation",
    "6-dof", "6dof", "6d pose", "multi-object tracking", "re-identification",
    "oriented object detection", "rotated object detection", "small object detection",
    "slam", "visual odometry", "lidar odometry", "loop closure", "point cloud",
    "autonomous navigation", "obstacle avoidance", "sentry", "radar station",
    "path planning", "motion planning", "trajectory optimization", "bird's eye view", "bev",
    "reinforcement learning", "sim-to-real", "imitation learning",
  ],
  low: [
    "robot", "robotic", "autonomous", "navigation", "perception", "detection",
    "tracking", "control", "drone", "uav", "quadruped", "legged", "manipulation",
    "yolo", "transformer", "real-time", "embedded", "ros2", "ros 2",
  ],
};
// 各档单条命中得分（命中按「是否出现」计一次，避免长文堆词刷分）。
export const RELEVANCE_TIER_SCORE = { high: 5, mid: 2, low: 1 };

// —— CVF openaccess 顶会论文爬取（需求1 选 c）——
// 直接从 openaccess.thecvf.com 抓取近年顶会论文清单，按 RM 关键词过滤后补入。
// 列表页只含标题/链接，命中关键词的少数论文再抓详情页取摘要。整体 best-effort，失败即跳过。
export const CVF_PROCEEDINGS = [
  "CVPR2026", "CVPR2025", "CVPR2024", "ICCV2025", "ICCV2023", "WACV2026", "WACV2025",
];
export const CVF_MAX = 20; // CVF 补入论文上限（避免占满）
...
// —— 限额 ——
export const PAPERS_MAX = 60; // 每次保留论文上限（按热度排序后截断，Q16）
export const PAPERS_CUMULATIVE_MAX = 100; // 当天累计论文天花板（增量合并后截断，Q26）
export const REPOS_MAX = 50; // 每次保留开源库上限（用户指定 50 个/次）
export const RECENT_DAYS_PAPERS = 30; // 优先保留近 N 天论文（Q17）
export const BACKFILL_DAYS_PAPERS = 90; // 近窗不足 PAPERS_MAX 时，可回溯到近 N 天补足（Q18）
export const ARXIV_MAX = 500; // 单分类 arXiv 抓取上限（Q20）
export const REPO_MIN_STARS = 10; // 开源库最低星标
export const FEATURED_TOP = 6; // 论文中标记为精选（生成精读）的数量（Q22）
export const TRANSLATE_TITLE_TOP = 40; // 预翻译标题的论文数量（Q23）
